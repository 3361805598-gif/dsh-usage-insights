...
function usageFrom(value) {
	...
	if (![
		...
		"cacheWriteTokens",
		"reasoningTokens"
	].some((key) => typeof raw[key] === "number" && Number.isFinite(raw[key]) && raw[key] >= 0)) return void 0;
...
function localParts(at, formatter) {
...
function selectedDays(now, formatter, range) {
	const [year, month, day] = localParts(now, formatter).day.split("-").map(Number);
...
function buildSummary(records, options) {
	...
	const calendar = options.range === "30d" ? calendarShell(days) : void 0;
...
const record = z.object({
	...
	reducerVersion: z.number().optional(),
	sessionId: z.string(),
...
var UsageInsightsIndex = class {
	...
	async start() {
		...
		this.refresh();
		this.ctx.effect(() => {
			const timer = setInterval(() => void this.refresh(), 6e4);
			timer.unref();
	...
	rebuild() {
		...
		this.index = {
			state: "indexing",
			processedSessions: 0,
			totalSessions: this.records.size,
			failures: 0
		};
	...
	refresh() {
		...
		this.refreshPending = this.enqueue(() => this.backfill()).finally(() => {
	...
	async prune() {
		...
		for (const record of this.records.values()) {
			...
			if (usage.length !== record.usage.length || skills.length !== record.skills.length) await this.save({
				...record,
				usage,
				skills
			});
	...
	installLiveReducer() {
		this.ctx.on("session/event", (session, event) => {
			if (event.type === "turn/end") this.enqueue(() => this.syncLive(session));
		});
	}
	async syncLive(session) {
		try {
			await this.ctx.sessions.flush(session);
			const snapshot = (await this.ctx.sessionPersistence.listSnapshots()).find((item) => item.header.id === session.id);
			if (!snapshot) return;
	...
	async backfill() {
		try {
			...
			for (const snapshot of snapshots) {
				...
				if (existing?.reducerVersion === 2 && existing.sourceCreatedAt === snapshot.header.createdAt && String(existing.sourceRevision) === revision) {
...
function isTrustedRequest(request, trustedHosts) {
	...
	try {
		...
		return (source.protocol === "http:" || source.protocol === "https:") && !source.username && !source.password && source.pathname === "/" && !source.search && !source.hash && source.host === hostUrl.host;
	} catch {
		return false;
	}
}
//#endregion
//#region src/index.ts
const name = "dsh-usage-analytics";
const inject = [
	"webServer",
	"webRuntime",
	"sessionPersistence",
...
async function apply(ctx) {
	...
	ctx.effect(() => ctx.webServer.register({
		...
		async handler(req, res) {
			...
			const path = url.pathname.slice(24);
